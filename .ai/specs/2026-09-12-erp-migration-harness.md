# One-Click ERP Migration (harness + NetSuite)

> Status: implemented
> Author: Claude
> Date: 2026-09-12 (restructured as a harness 2026-09-13)
> Research: `packages/migration/gaps/netsuite.md` (generated), Oracle NetSuite SuiteTalk REST / SuiteQL documentation
...
A harness plus one directory per system, split so the interesting part is testable
without a source account or a database:
...
Carbon item type, which orders are in scope — lives in the source's `map/`, in one
reviewable place. Adding a second source adds no code to the job, the loader or the page.

### The job

`carbon/migration`, modelled directly on `company-template`:

1. Guard: refuse a second run while one is pending review.
2. Marker row (`externalIntegrationMapping`, `integration = 'migration'`) with
...
`carbon/migration-finalize` (Keep/Dismiss) and `carbon/migration-revert` share a
...
**D5 — Idempotent by external id, namespaced by source.** Every migrated record is linked
in `externalIntegrationMapping` under `integration = <sourceId>` — `netsuite`, not
`migration` — so a second run updates rather than duplicates, and two sources migrating
into one company cannot collide on the same record id. The one thing with no natural key —
...
FRAMEWORK is the harness's; the entries belong to the source, because what cannot come
across is a property of the system you are leaving. Extraction counts what
...
31 catalogued gaps, in `packages/migration/gaps/netsuite.md`. The six rated high: GL transaction
...
- `pnpm --filter @carbon/migration test` — tests over the client, the TBA signer, the
  mappers, the loader helpers, the source registry and the gap register.
- `pnpm exec turbo run typecheck --filter=erp --filter=@carbon/jobs --filter=@carbon/migration`
...
- A source reads its whole account into memory before loading. That is fine at the
