...
- **Onshape panel auth** (`./onshape/panel-session.server`, `src/onshape/panel/session.server.ts`): the `cps_` bearer credential for the cross-site Onshape iframe (the `carbon` cookie is `SameSite=Lax` and can't reach it). Mint/store/refresh primitives (`createPanelSession`/`deletePanelSession`/`panelSessionTokenFromRequest`, Redis `panel-session:<token>`, 12h TTL) plus `requireOnshapePanelPermissions` — the panel routes' OWN gate, mirroring `requirePermissions`' claims check + RLS client but with no cookie/redirect path (401/403 only). Lives here, not in `@carbon/auth`, on purpose: it is an integration concern. Imports `refreshAccessToken`/`getUserClaims` from `@carbon/auth`; auth never imports it back. See `.claude/rules/onshape-panel.md`
- **Exports**: `./accounting`, `./planning`, `./plan`, `./plan.server`, `./slack.server`, `./hooks.server`, `./sso.server`, `./onshape/panel-session.server`, `./jira`, `./linear`, `./rillet/hooks.server`, `./xero/hooks.server`, etc.
