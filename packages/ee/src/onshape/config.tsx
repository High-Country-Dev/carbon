...
export const Onshape = defineIntegration({
  ...
  images: [],
  /*
   * Push defaults: what a panel push fills in that Onshape has no field for.
   * Everything else a created item carries — part number, name, description,
   * revision, Buy vs Make — comes from Onshape and is changed there. Read at
   * plan time by `parsePushDefaults`, which falls back to DEFAULT_PUSH_DEFAULTS
  ...
  schema: z
    .object({
      // SwitchField posts a literal "true"/"false" string; preprocess explicitly
      // so unchecking sticks (z.coerce.boolean would treat "false" as truthy).
      assetSyncEnabled: z
        .preprocess((value) => {
