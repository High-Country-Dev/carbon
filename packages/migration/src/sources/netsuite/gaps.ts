import type { MigrationGapDefinition } from "../../gaps/index.ts";

/**
 * Everything a NetSuite → Carbon migration does not carry across, or carries
 * across differently. Source of truth: `gaps/netsuite.md` is generated from this
 * list and `sources/gaps.test.ts` fails if the two drift.
 *
 * Ids are stable and never renumbered — the run report, the docs site and the
 * support playbook all reference them. The `NS-` prefix is this source's; a new
 * source gets its own.
 */
export const NETSUITE_GAPS: MigrationGapDefinition[] = [
  // ── Accounting ────────────────────────────────────────────────────────────
  {
    id: "NS-ACC-001",
    area: "accounting",
