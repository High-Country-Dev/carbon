...
import { bool, date, num, type SourceRow, str } from "../../../values.ts";
...
function entityName(row: SourceRow): string {
...
function addressesFor(rows: SourceRow[], entityId: string): PlanAddress[] {
  return rows
...
function contactsFor(rows: SourceRow[], entityId: string): PlanContact[] {
  return rows
...
function groupTransactions(rows: SourceRow[]): Map<string, SourceRow[]> {
...
export function mapSnapshotToPlan(snapshot: NetSuiteSnapshot): MapResult {
  ...
  const bomByAssembly = new Map<
    string,
    { externalId: string; lines: SourceRow[] }
  >();
  ...
  const gaps = detectGaps(NETSUITE_GAPS, {
