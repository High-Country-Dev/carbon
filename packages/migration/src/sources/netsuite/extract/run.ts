import { ScopeChoiceRequired } from "../../../errors.ts";
import type { SourceRow } from "../../../values.ts";
...
export type NetSuiteSnapshot = {
  ...
  currencies: SourceRow[];
  unitsOfMeasure: SourceRow[];
  paymentTerms: SourceRow[];
  shippingMethods: SourceRow[];
  locations: SourceRow[];
  departments: SourceRow[];
  accounts: SourceRow[];
  customerCategories: SourceRow[];
  vendorCategories: SourceRow[];
  customers: SourceRow[];
  customerAddresses: SourceRow[];
  vendors: SourceRow[];
  vendorAddresses: SourceRow[];
  contacts: SourceRow[];
  items: SourceRow[];
  itemVendors: SourceRow[];
  itemPrices: SourceRow[];
  bomLines: SourceRow[];
  inventoryBalances: SourceRow[];
  /** Header columns repeated per line; the mapper groups them by `id`. */
  salesOrderLines: SourceRow[];
  purchaseOrderLines: SourceRow[];
...
/**
 * A NetSuite subsidiary is this source's "scope" — the unit that maps 1:1 to a
 * Carbon company. Raised so the harness can render the choice; the extractor
 * must not make it, because merging subsidiaries double-counts every
 * intercompany transaction.
 */
function subsidiaryChoiceRequired(
  subsidiaries: AccountProbe["subsidiaries"]
): ScopeChoiceRequired {
  return new ScopeChoiceRequired(
    "netsuite",
...
export async function extractNetSuite(
  ...
  if (probe.oneWorld) {
    if (subsidiaryId === null && migratable.length > 1) {
      throw subsidiaryChoiceRequired(migratable);
    }
