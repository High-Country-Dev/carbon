...
/**
 * A migrated record is filed in `externalIntegrationMapping` under the SOURCE's
 * id — `netsuite`, not `migration`. That is what makes a re-run an UPDATE
 * instead of a duplicate, and what lets a migrated record be traced back to the
 * system it came from years later. Two sources migrating into one company
 * therefore keep separate namespaces, which is the point: the same external id
 * can mean different records in different systems.
 */
...
/**
 * Source record id → Carbon id, for one migration run.
 *
...
export async function readExistingMappings(
  db: LoadDb,
  args: { companyId: string; sourceId: string }
  ...
  const rows = await db
    ...
    .where("integration", "=", args.sourceId)
