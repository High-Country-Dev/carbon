 ...
 *
 * Both carry their source transaction number as Carbon's document number, so
  ...
  for (const order of ctx.plan.salesOrders) {
    ...
    const inserted = await ctx.trx
      ...
      .values({
        ...
        // already made in the source system, and importing it as a Draft would hide it
    ...
    for (const line of order.lines) {
      ...
      const lineRow = await ctx.trx
        ...
        .values({
          ...
          // original quantity would re-promise what the source already shipped.
