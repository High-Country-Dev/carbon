...
import { PLAN_SECTIONS } from "../plan.ts";

import type { MigrationIdMap } from "./mapping.ts";
...
export type CompanyConfig = {
  ...
  timezone: string;
  /** Carbon's own location, used when a source record names none. */
...
export type LoadCtx = {
  ...
  companyId: string;
  userId: string;
  /** Which system this plan came from — it namespaces every external-id link. */
  sourceId: string;
  plan: MigrationPlan;
  ids: MigrationIdMap;
...
export async function readCompanyConfig(
  ...
  if (!defaultLocationId) {
    throw new Error(
      `Company ${companyId} has no location — finish onboarding before migrating into it`
    );
  }
