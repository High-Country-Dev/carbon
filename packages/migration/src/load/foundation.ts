import type { CarbonAccountClass, CarbonAccountType } from "../plan.ts";

import type { LoadCtx } from "./context.ts";
import { lookupKey } from "./context.ts";

/**
 * Tier 1 — the lookups every later tier resolves against.
 *
 * Each of these MERGES: a source value that already exists in Carbon under the
...
export async function loadCurrencies(ctx: LoadCtx): Promise<void> {
  for (const currency of ctx.plan.currencies) {
    ...
    const inserted = await ctx.trx
      .insertInto("currency")
      .values({
        // `currency` carries no name column — the plan keeps the source's name
  ...
  for (const location of ctx.plan.locations) {
    ...
    const inserted = await ctx.trx
      ...
      .values({
        ...
        // source's location record does not always carry. An empty string keeps
        ...
        // sane answer — source locations rarely carry one.
...
/**
 * Carbon's account type, derived from the account's class when the source's own
 * type has no Carbon counterpart. Only ever a fallback — a source's mapper sets
 * the type directly whenever its own account type maps cleanly.
 */
  ...
  if (groupMembers.length > 1) {
    ctx.warn(
      `Accounts are shared across the ${groupMembers.length} companies in this group — ` +
        `${ctx.plan.accounts.length} migrated accounts were merged into the shared chart.`
    );
  }

  // Insert parents before children so `parentId` always resolves; a source
  // returns its chart in no particular order.
