...
/** The unit code to write, falling back to the company's `EA` when the source's is unknown. */
...
export async function loadOpeningStock(ctx: LoadCtx): Promise<void> {
  ...
  // Namespaced by source: migrating from a second system later must not think
  // the first system's opening entries were its own and skip real stock.
  const markerPrefix = `migration:${ctx.sourceId}:opening:`;
  const markerFor = (
    itemExternalId: string,
    locationExternalId: string | null
  ) => `${markerPrefix}${itemExternalId}:${locationExternalId ?? "default"}`;
  ...
  const existingMarkers = new Set(
    (
      await ctx.trx
        ...
        .where("companyId", "=", ctx.companyId)
        .where("externalDocumentId", "like", `${markerPrefix}%`)
        .execute()
    )
  ...
  for (const stock of ctx.plan.openingStock) {
    ...
    await ctx.trx
      ...
      .values({
        ...
        comment: `Opening balance migrated from ${ctx.sourceId}`,
        companyId: ctx.companyId,
        createdBy: ctx.userId,
        createdAt: ctx.now
      })
