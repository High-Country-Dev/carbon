...
import type { MigrationPlan, PlanSection } from "../plan.ts";
import {
  type CompanyConfig,
  emptyCounts,
  type LoadCtx,
...
import { loadCustomers, loadSuppliers } from "./parties.ts";

export type LoadOptions = {
  companyId: string;
  userId: string;
  /**
   * The source the plan came from. It namespaces the external-id links, so two
   * sources migrating into one company cannot collide on the same record id.
   */
  sourceId: string;
  plan: MigrationPlan;
  /**
 ...
 * `externalIntegrationMapping` under the source's id, so a second run over the
 * same source account updates what it created and inserts only what is new. The one thing with no
...
export async function loadMigrationPlan(
  ...
  const { companyId, userId, sourceId, plan, onProgress, log } = options;
  ...
  const ids: MigrationIdMap = await readExistingMappings(trx, {
    companyId,
    sourceId
  });
...
export type { LoadCtx, LoadTx, SectionCounts } from "./context.ts";
