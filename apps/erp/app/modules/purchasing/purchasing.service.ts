...
// ---------------------------------------------------------------------------
// Ordinary CRUD. Bank detail changes are the classic payment-fraud vector, so a change
// control — approval before a new account becomes payable, an alert on an edit — is worth
// having, but it belongs with the surface that spends the money. Nothing reads these rows
// yet; see .ai/specs/2026-07-04-master-data-controls.md.
...
export async function upsertSupplierBankAccount(
  client: SupabaseClient<Database>,
  bankAccount: Omit<z.infer<typeof supplierBankAccountValidator>, "id"> & {
    companyId: string;
    supplierId: string;
    customFields?: Json;
  ...
  if ("createdBy" in bankAccount) {
    return client
      .from("supplierBankAccount")
      .insert([
        { ...row, companyId, supplierId, createdBy: bankAccount.createdBy }
      ])
      .select("id")
      .single();
  }
