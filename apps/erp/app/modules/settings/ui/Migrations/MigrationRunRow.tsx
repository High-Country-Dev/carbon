...
export function MigrationRunRow({
  ...
  run: MigrationRun;
  /** The source's display name, so the row never says "netsuite". */
  sourceName: string;
  ...
  const phaseLabel: Record<string, string> = {
    connect: t`Connecting to ${sourceName}`,
    extract: t`Reading your ${sourceName} account`,
    map: t`Matching ${sourceName} records to Carbon`,
  ...
  const title = isReverting
    ...
    : run.status === "running"
      ? run.dryRun
        ? t`Previewing your ${sourceName} data`
        : t`Migrating from ${sourceName}`
      : run.status === "failed"
      ...
      <CardHeader>
        ...
        <CardDescription>
          {busy ? (
            ...
            <Trans>
              Your data is in Carbon. Keep it, or revert to put back exactly
            ...
            <VStack spacing={0} className="min-w-0">
              <span className="text-sm font-medium truncate">
                {run.report?.accountId
                  ? t`${sourceName} account ${run.report.accountId}`
                  : sourceName}
